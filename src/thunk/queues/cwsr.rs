...
fn get_vgpr_size_per_cu(gfx_version: u32) -> u32 {
    ...
    let is_large_vgpr_gfx9 = major == 9
        && (
            (minor == 4 && step == 2) ||    // Aldebaran
            (minor == 0 && step == 8) ||    // Arcturus
            (minor == 4) ||                 // Aqua Vanjaram family
...
fn cntl_stack_bytes_per_wave(gfx_version: u32) -> u32 {
    let major = (gfx_version / 10000) % 100;
...
pub fn calculate_sizes(props: &HsaNodeProperties) -> Option<CwsrSizes> {
    ...
    // Navi10+: cu_num * 32
    let wave_num = if props.gfx_target_version < 100100 {
        // Pre-Navi10 (10.1.0)
    ...
    if props.gfx_target_version == 100100 {
