...
impl EventManager {
    ...
    pub fn create_event(
        ...
        // KFD uses 'auto_reset' logic, so we invert the boolean.
        ...
        if self.events_page.is_none() {
            ...
            // Allocate system-coherent, page-aligned memory.
            let events_alloc = memory_manager
                .allocate_exec_aligned_memory_gpu(device, alloc_size, 4096, desc.node_id, drm_fd)
                .inspect_err(|&e| {
        ...
        let event = HsaEvent {
            ...
            last_event_age: 0, // Age starts at 0
        };
            ...
            match event.event_type {
                ...
                HsaEventType::Memory => unsafe {
                    ...
                    if ioctl_evt.payload.memory_exception_data.gpu_id != 0 {
                        ...
                        // KFD reported memory faults are unrecoverable/fatal.
                        let is_fatal = true;
                ...
                _ => {
                    // For System events or others without "age", the successful return
                    // of the IOCTL implies they triggered if we aren't using `wait_all`.
                    // We optimistically mark them as signaled.
                    signaled_indices.push(i);
                }
            }
        }
